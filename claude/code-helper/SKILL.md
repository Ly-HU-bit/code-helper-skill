...
Claude Code expands `${CLAUDE_SKILL_DIR}` to this skill directory. Use an available Python 3 interpreter.

## Workflow

1. Prepare one clean, runnable workspace and reuse its manifest throughout the task:

   `python "${CLAUDE_SKILL_DIR}/scripts/codehelper_cli.py" prepare input/<task> output/<task>/code --profile <profile> -o output/<task>/manifest.json`
...
7. Finalize compact output with:
   ...
   `python "${CLAUDE_SKILL_DIR}/scripts/codehelper_cli.py" finalize input/<task> output/<task>/code output/<task> --profile <profile>`
